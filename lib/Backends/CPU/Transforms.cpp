...
static Node *useMaltempiConv(ConvolutionNode *CN, Function *F) {
  auto *M = F->getParent();

  // Only available when MO436-features is turned on.
  if (!MO436Features) {
    return nullptr;
  }
  ...
  return F->addNode(new MaltempiConvNode(
...
CPUBackend::transformPostLowering(Function *F, CompilationContext &,
  ...
  for (auto &node : F->getNodes()) {
    ...
    if (auto *CN = dyn_cast<ConvolutionNode>(&node)) {
      if (Node *NCN = useMaltempiConv(CN, F)) {
        CN->getResult().replaceAllUsesOfWith(NCN);
        changed = true;
        continue;
      }
